...
if (!isWindows) {
  ...
  console.log('• Skip PDF export (Windows)');
}
...
if (!isWindows) {
  ...
  console.log('• Sizes OK: md', fs.statSync(mdPath).size, '(PDF skipped on Windows)');
}
...
const artifacts = { md:{ path:mdPath, size:fs.statSync(mdPath).size, sha256:fileHash(mdPath)}, html:{ path:htmlPath, size:fs.statSync(htmlPath).size, sha256:fileHash(htmlPath)} };
if (!isWindows) {
  artifacts.pdf = { path:pdfPath, size:fs.statSync(pdfPath).size, sha256:fileHash(pdfPath) };
}
